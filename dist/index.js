...
var Stack = _Stack;
var _a, _b, _max, _maxSize, _dispose, _onInsert, _disposeAfter, _fetchMethod, _memoMethod, _size, _calculatedSize, _keyMap, _keyList, _valList, _next, _prev, _head, _tail, _free, _disposed, _sizes, _starts, _ttls, _hasDispose, _hasFetchMethod, _hasDisposeAfter, _hasOnInsert, _LRUCache_instances, initializeTTLTracking_fn, _updateItemAge, _statusTTL, _setItemTTL, _isStale, initializeSizeTracking_fn, _removeItemSize, _addItemSize, _requireSize, indexes_fn, rindexes_fn, isValidIndex_fn, evict_fn, backgroundFetch_fn, isBackgroundFetch_fn, connect_fn, moveToTail_fn, delete_fn, clear_fn;
  ...
  static unsafeExposeInternals(c) {
    return {
      ...
      backgroundFetch: (k, index, options, context2) => {
        var _a3;
        return __privateMethod(_a3 = c, _LRUCache_instances, backgroundFetch_fn).call(_a3, k, index, options, context2);
      },
      moveToTail: (index) => {
        var _a3;
  ...
  *rvalues() {
  ...
  async fetch(k, fetchOptions = {}) {
    ...
    if (index === void 0) {
      if (status)
        status.fetch = "miss";
      const p = __privateMethod(this, _LRUCache_instances, backgroundFetch_fn).call(this, k, index, options, context2);
...
backgroundFetch_fn = function(k, index, options, context2) {
...
Processing tag: ${tag}`);
      ...
      try {
        ...
        if (error2 instanceof Error) {
          if ("status" in error2 && error2.status === 422 && error2.message.includes("already_exists")) {
            core.warning(`\u26A0\uFE0F  Release for ${tag} already exists`);
          } else {
            core.error(
              `\u274C Failed to create release for ${tag}: ${error2.message}`
            );
          }
        }
      }
    ...
    if (error2 instanceof Error) {
      core.setFailed(`Error creating releases: ${error2.message}`);
    } else {
      core.setFailed(`Error creating releases: ${error2}`);
    }
  }
}
main();
